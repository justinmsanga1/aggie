...
const SellSlot = ({ onComplete }) => {
  ...
  const chooseSlot = (account, consoleType) => {
    ...
    setForm((prev) => ({ ...prev, price: consoleType === 'ps5' ? '4.00' : '3.50' }));
  };
  ...
  return (
    <div className="nexus-page sell-page fade-in">
      <header className="page-top"><div><span className="eyebrow">Fast sale</span><h1>Sell Slot</h1><p>Start with the game the customer wants.</p></div>{selectedGame && <button className="icon-shell" onClick={()=>{setSelectedGame(null); setSelected(null);}}><ChevronLeft size={21}/></button>}</header>
