...
const Accounts = ({ onViewDetails }) => {
  ...
  return (
    <div className="nexus-page accounts-page fade-in">
      ...
      <section className="control-card"><div className="search-control"><Search size={17}/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search email, game, region..."/></div><div className="chip-scroll">{filters.map(([id,label])=><button key={id} className={filter===id?'active':''} onClick={()=>setFilter(id)}>{label}</button>)}<button type="button" className="icon-shell" style={{marginLeft:'auto'}} onClick={openAddAccount} aria-label="Add account"><Plus size={18}/></button></div></section>
