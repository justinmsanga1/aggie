...
const Accounts = ({ onViewDetails }) => {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      alert(error.message || 'Could not add account. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="nexus-page accounts-page fade-in">
      ...
      {open && <section className="inline-add-account"><div className="inline-add-head"><div><h3>Buy / Add Account</h3><p>Record the account purchase and attach one or more games from database.</p></div><button type="button" onClick={resetAddForm} disabled={saving}>Close</button></div><form key={open ? 'add-open' : 'add-closed'} onSubmit={submit}><div className="form-group"><label className="form-label">Email</label><input className="form-input" name="email" type="email" required autoFocus/></div><div className="form-group"><label className="form-label">Password</label><input className="form-input" name="password" type="text" placeholder="PSN account password"/></div><div className="form-group"><label className="form-label">Region</label><select className="form-select" name="region"><option>US</option><option>UK</option><option>TR</option><option>JP</option></select></div><div className="form-group"><label className="form-label">Cost (TZS)</label><input className="form-input" name="cost" type="number" step="1" min="0" required/></div><div className="form-group"><label className="form-label">Games on this account</label>{games.length ? <div className="game-check-list">{games.map(g=><label key={g.id} className="game-check-row"><input type="checkbox" checked={selectedGames.includes(g.id)} onChange={()=>toggleGame(g.id)}/><span>{g.name}</span></label>)}</div> : <p className="empty-line">No games yet. Type a new game name below.</p>}</div><div className="form-group"><label className="form-label">+ Add new game names</label><div className="dynamic-game-inputs">{newGameNames.map((val, i)=><div key={i} className="game-input-row"><input className="form-input" value={val} onChange={(e)=>{const next=[...newGameNames]; next[i]=e.target.value; setNewGameNames(next);}} placeholder="e.g. FIFA 26 Ultimate Edition"/>{newGameNames.length>1 ? <button type="button" className="icon-shell game-input-remove" onClick={()=>setNewGameNames(newGameNames.filter((_,j)=>j!==i))}><X size={16}/></button> : null}</div>)}<button type="button" className="game-input-add" onClick={()=>setNewGameNames([...newGameNames, ''])}><Plus size={16}/> Add game</button></div></div><div className="form-group"><label className="form-label">Notes</label><textarea className="form-textarea" name="notes"/></div><button type="submit" className="sheet-submit-btn" disabled={saving}>{saving ? 'Saving...' : 'Add account'}</button></form></section>}
      <section className="control-card"><div className="search-control"><Search size={17}/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search email, game, region..."/></div><div className="control-row"><div className="chip-scroll">{filters.map(([id,label])=><button key={id} className={filter===id?'active':''} onClick={()=>setFilter(id)}>{label}</button>)}</div><button type="button" className="icon-shell" onClick={openAddAccount} aria-label="Add account"><Plus size={18}/></button></div></section>
