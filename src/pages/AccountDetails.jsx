...
const AccountDetails = ({ id, accountId, onBack }) => {
  ...
  return <div className="nexus-page details-page fade-in">
    <header className="details-top"><button className="icon-shell" onClick={onBack}><ArrowLeft size={20}/></button><button className="icon-shell"><Copy size={19}/></button></header>
    <section className="detail-hero"><div className="detail-meta"><span><Shield size={14}/>{account.status}</span><span><Globe2 size={14}/>{account.region}</span><span><Clock3 size={14}/>{account.nextDeactivation || 'No reset date'}</span></div><div className="detail-money"><div><small>Total spent</small><b className={stats.profit>=0?'positive':'negative'}>{money(stats.profit)}</b></div><div><small>Total invested</small><b>{money(stats.totalInvested)}</b></div><div><small>Revenue</small><b>{money(account.revenue)}</b></div><div><small>PSN left</small><b>{money(stats.psnBalance)}</b></div></div></section>
